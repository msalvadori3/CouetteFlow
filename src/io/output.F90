MODULE output_m
      ...
      USE SimVars_m,ONLY:jmax,y,yp,u,up,u_exact,up_exact
...
CONTAINS
      ...
      SUBROUTINE WritePlotFile(fileName,varNames,inputData,t)
              ...
              IF (fileName == 'None' .OR. fileName == 'none') THEN
                      file_name1 = 'output/' // 'Couette_'&
                              //'_t_'//num2str(t,'(F15.3)',20)
              ELSE
                      file_name1 = 'output/'//TRIM(fileName)//'_t_'//&
                              num2str(t,'(F15.3)',20)
              ENDIF
              ...
              IF (inputData%PostProcessing%plot%iPost == 1 .AND. &
                      ...
                      WRITE(*,*) 'Printing Results for TecPlot @ t =',t,'s'
                      ...
                      WRITE(IOunit,'(A)') ""
                      WRITE(IOunit,'(A,I6,A,I6,A)') 'Zone J=',&
                                                           jmax, ', F=POINT'

                      DO j = 1, jmax
                          WRITE(IOunit,'(6g15.6)') y(j), u(j),u_exact(j),yp(j),up(j),up_exact(j)
                      ENDDO
              ...
              ELSE IF (inputData%PostProcessing%plot%iPost == 1 .AND. &
                      ...
                      WRITE(*,*) 'Priting Results for Python @ t = ',t,'s'
      ...
      SUBROUTINE WriteRMS(niter,rmsSS,rmsUS,fileName,inputData)
              ...
              TYPE(input_type_t),INTENT(IN) :: inputData
              CHARACTER(LEN=*),INTENT(IN) :: fileName
              REAL(KIND=wp),INTENT(IN) :: rmsSS,rmsUS
              ...
              IF (fileName == 'None' .OR. fileName == 'none') THEN
                      file_name1 = 'output/' // 'RMSout_'//&
                              num2str(inputData%setup%dt,'(F13.5)',15)//&
                              '_dt_'//num2str(inputData%setup%theta,&
                              '(F5.5)',5)

              ELSE
                      file_name1 = 'output/'//TRIM(fileName)//'_dt_'//&
                              num2str(inputData%setup%dt,'(F13.5)',15)//&
                              '_theta_'//num2str(inputData%setup%theta,&
                              '(F3.1)',5)
              ...
              IF (niter == 1) THEN
                      ...
                      WRITE(IOunit,'(A,5X,A,5X,A)') 'Iter','SteadyRMS','UnsteadyRMS'
              ...
              WRITE(IOunit,'(I6,G15.6,G15.6)')niter,rmsSS,rmsUS
              CLOSE(IOunit)
