...
rmsFile = 'rms_dt_.0002_theta_0.0.dat'
rmsFile = os.path.join(dataDir,rmsFile)
rmsdata = np.loadtxt(rmsFile,skiprows=1,unpack=True)
...
plt.title("$\Theta = 0$ with dt=0.00021",fontsize=16)
plt.setp(p, linewidth='1.0')
plt.axis([MinX,1000, MinY, 10e20])
plt.xscale('linear')
plt.yscale('log')
